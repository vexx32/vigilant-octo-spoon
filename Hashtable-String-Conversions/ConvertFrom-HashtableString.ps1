...
.DESCRIPTION
    Takes an input string or file path, and invokes it as an expression, casting it to a hashtable 
    object. Will throw an error on any non-hashtable object sequences, but any arbitrary commands may 
    be present in the hashtable value fields. Use with caution.
.EXAMPLE
    PS C:\> ConvertFrom-HashtableString -FilePath .\hashtable.txt
    
    Takes the file at the path specified (in this case, 'C:\hashtable.txt') and parses the text into a 
    useable PowerShell object.
.INPUTS
